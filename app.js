...
const elements = {
  ...
  todoLevel: document.getElementById("todoLevel"),
...
function loadTodos() {
  ...
  try {
    ...
    if (!Array.isArray(parsed)) {
      throw new Error("Stored todos were not an array.");
    }

    const normalizedTodos = parsed
      .map(normalizeTodo)
...
function handleAddTodo(event) {
  ...
  state.todos.unshift({
    ...
    level: normalizeLevel(elements.todoLevel.value),
  });

  elements.todoInput.value = "";
  elements.todoLevel.value = DEFAULT_LEVEL;
  saveTodos();
  render();
}
...
function editTodo(id) {
  ...
  const nextText = window.prompt(
    `Edit your ${formatLevel(todo.level)} task:`,
    todo.text
  );
...
function render() {
  ...
  visibleTodos.forEach((todo) => {
    ...
    const levelBadge = fragment.querySelector(".level-badge");
    ...
    levelBadge.textContent = formatLevel(todo.level);
    levelBadge.classList.add(`level-badge--${todo.level}`);
...
function formatLevel(level) {
  return normalizeLevel(level).replace(/^./, (value) => value.toUpperCase());
}
...
function normalizeTodo(todo) {
  return {
    ...
    level: normalizeLevel(todo?.level),
  };
}
